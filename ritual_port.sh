...
if docker ps -a --filter "name=^/${CONTAINER_NAME}$" --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
  ...
  if [ -f "$CONFIG_FILE" ]; then
    sed -i 's/3017/3000/g' "$CONFIG_FILE"
    sed -i 's/3017/3000/g' "$CONFIG_FILE_OWN"
    echo "Порт изменен с 3017 на 3000 в файле $CONFIG_FILE."
    echo "Стартую hello-world на 3000 порте."
